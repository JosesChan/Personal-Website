    ...
    <nav className="bg-primary lg:px-5 sm:px-4 py-3 fixed w-full z-20 top-0 left-0 border-b border-secondary">
      <div className="container flex flex-wrap justify-between items-center mx-auto w-10/12">
        ...
        <div className="md:hidden order-2">
          <button className="p-2 text-character rounded-none outline-none focus:border-secondary focus:border" onClick={() => setNavbar(!navbar)}>
            {navbar ? (
        ...
        {/*Social Media Items*/}
        <div className="flex flex-row order-3 hidden md:flex items-center space-x-3 flex-row">
          <a href="https://github.com/JosesChan?tab=repositories" target="_blank" rel="noopener noreferrer"
            className="text-character opacity-60 hover:opacity-100 transition-opacity duration-300" aria-label="GitHub">
          ...
          <a href="https://www.linkedin.com/in/fong-sun-joses-chan-b693667b/" target="_blank" rel="noopener noreferrer"
            className="text-character opacity-60 hover:opacity-100 transition-opacity duration-300" aria-label="Linkedin">
            <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="currentColor" viewBox="0 0 24 24">
